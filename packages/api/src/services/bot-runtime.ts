...
function titleFromProgressInput(input: HermesPlatformProgressInput): string | null {
  if (input.type !== "session.title") return null;
  return normalizeHermesGeneratedTitle(input.payload?.title);
}
...
async function prepareHermesPlatformEvent(
  ...
  const sessionIntent = loaded.thread?.branchPending
    ? {
        type: "branch",
        fromThreadId: loaded.thread.branchFromThreadId ?? null,
...
async function failInvocation(
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const event = createChatMessageSentV1({
      ...
      occurredAt: responseMessage.createdAt,
    });
    await enqueueDomainEvent(tx, event, {
      partitionKey: responseMessage.conversationId,
    });
