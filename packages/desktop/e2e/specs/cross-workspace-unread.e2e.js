// Native E2E coverage for durable unread activity across workspaces.
//
// The test disconnects the recipient, sends a message in an inactive
// workspace, signs the recipient back in, and verifies that both the Activity
// inbox and workspace/channel markers hydrate from server-owned unread state.
...
describe("Cross-workspace unread activity", function () {
  ...
  it("surfaces an unread inactive workspace before it is selected", async () => {
    ...
    const offlineMessage = "Unread while the desktop is disconnected";
    const sent = await browser.execute(
      async (apiUrl, token, conversationId, content) => {
    ...
    if (!sent.ok) {
      throw new Error(`Sending the offline Beta message failed: ${JSON.stringify(sent)}`);
    }
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 15_000,
        timeoutMsg: "Workspace Alpha was not restored after login",
      },
    );
    ...
    const activityButton = await $("button[aria-label^='Activity,']");
    await activityButton.waitForClickable({ timeout: 10_000 });
    await activityButton.click();
    const activityHeading = await $("h1=Activity");
    await activityHeading.waitForExist({ timeout: 10_000 });
    await browser.waitUntil(
      async () => (await $("body").getText()).includes(offlineMessage),
      {
        timeout: 10_000,
        timeoutMsg: "Activity did not show the persisted offline message",
      },
    );
