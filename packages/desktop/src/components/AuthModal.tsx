...
const useAuthModalState = create<AuthModalState>(() => ({
  open: false,
  initialMode: "login",
}));

export const openAuthModal = (mode?: AuthMode) =>
  useAuthModalState.setState({
    open: true,
...
export function AuthModal() {
  const { open, initialMode } = useAuthModalState();
  if (!open) return null;
  return <AuthModalContent initialMode={initialMode} />;
}

function AuthModalContent({ initialMode }: { initialMode: AuthMode }) {
  useEffect(() => {
  ...
  return (
    <AuthOverlay onCancel={closeAuthModal}>
      <AuthPanel
