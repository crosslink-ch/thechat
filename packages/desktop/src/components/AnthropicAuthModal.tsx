      ...
      <div className="w-full max-w-[420px] rounded-xl border border-border-strong bg-surface p-6 shadow-card animate-slide-up" onClick={(e) => e.stopPropagation()}>
              ...
              <>
                ...
                <div className="mb-3 flex gap-2">
                  <input
                    ...
                    className="min-w-0 flex-1 rounded-lg border border-border bg-raised px-3 py-2 font-mono text-[12px] text-text outline-none transition-colors placeholder:text-text-dimmed focus:border-accent"
                    spellCheck={false}
                    disabled={submitting}
                  />
                  <button
                    className="shrink-0 cursor-pointer rounded-lg border-none bg-accent px-4 py-2 text-[12px] font-medium text-white transition-colors hover:not-disabled:bg-accent-hover disabled:cursor-not-allowed disabled:opacity-50"
        ...
        {status === "authenticated" && (
          <>
            ...
            <p className="mb-4 text-[13px] leading-relaxed text-text-muted">
              Set <code className="rounded-md bg-base px-1.5 py-0.5 text-[11px]">"provider": "anthropic"</code> in your config.json to route messages through Claude.
            </p>
