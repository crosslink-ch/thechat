...
export default defineConfig({
  root: here("."),
  define: { __WEB_BUILD__: false, __WEB_API_URL__: JSON.stringify(""), __WEB_WS_URL__: JSON.stringify(""), __BACKEND_URL__: JSON.stringify(process.env.THECHAT_BACKEND_URL || `http://localhost:${process.env.THECHAT_BACKEND_PORT || "3000"}`) },
  plugins: [react()],
  resolve: { alias: {
