...
it("collapses navigation before Tasks and restores both when widening", async () => {
  ...
  for (const width of [1440, 1280, 1279, 1100, 1024, 1023, 390, 1024, 1279, 1280]) {
    resize(width);
    ...
    expect(Boolean(channel), `inline navigation at ${width}px`).toBe(width >= 1280);
    expect(Boolean(navigation), `navigation trigger at ${width}px`).toBe(width < 1280);
    expect(Boolean(tasks), `inline Tasks at ${width}px`).toBe(width >= 1024);
    expect(Boolean(tasksToggle), `Tasks trigger at ${width}px`).toBe(width < 1024);
  }
