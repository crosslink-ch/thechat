...
describe("ChannelChatView", () => {
  ...
  it("highlights messages that mention the signed-in user", () => {
    ...
    const { container } = render(
      <ChannelChatView
        messages={[
          message({ id: "a", senderId: "user-2", senderName: "Bruno", senderType: "human", content: "@Tony I was not online" }),
          message({ id: "b", senderId: "user-2", senderName: "Bruno", senderType: "human", content: "No mention here", createdAt: "2026-01-01T01:00:00.000Z" }),
        ]}
        loading={false}
        typingUsers={new Map()}
        onSend={() => {}}
        mentions={[{ id: "user-2", label: "Bruno", type: "human" }]}
      />,
    );

    expect(container.querySelector('[data-message-id="a"]')).toHaveAttribute("data-mentions-you", "true");
    expect(container.querySelector('[data-message-id="b"]')).not.toHaveAttribute("data-mentions-you");
    expect(container.querySelector(".md-mention-self")).toHaveTextContent("@Tony");
  });
