...
async function playRealAudio(page: Page, selector: string) {
...
test('real web voice: cancel, preview, channel and DM send, reload, playback and byte-exact download', async ({ page, request }, info) => {
  ...
  for (const [kind, id] of [['channel', f.channel.id], ['dm', f.dm.id]] as const) {
    await test.step(`${kind}: preview is local; attach/send persists playable, downloadable bytes`, async () => {
      ...
      await page.getByRole('button', { name: 'Record voice message', exact: true }).click();
      await expect(page.getByRole('status').filter({ hasText: /Recording 0:02/ })).toBeVisible();
      ...
      const previewPlayback = await playRealAudio(page, 'audio[aria-label="Voice message preview"]');
      ...
      await assertContained(page, 'audio[aria-label="Voice message preview"]');
      ...
      await expect(page.getByTitle('Send message', { exact: true })).toBeEnabled({ timeout: 45_000 });
      ...
      expect(await messages(id), 'Attach is not Send').toHaveLength(0);
      ...
      const load = page.getByRole('button', { name: 'Load audio voice-message.webm', exact: true });
      await expect(load).toBeVisible();
      expect(traffic.authorizations).toBe(baseline.authorizations);
      await page.reload();
      await expect(load).toBeVisible();
      await expect(page.getByLabel('Audio: voice-message.webm', { exact: true })).toHaveCount(0);
      expect(traffic.authorizations).toBe(baseline.authorizations);
      await load.click();
      const audio = page.getByLabel('Audio: voice-message.webm', { exact: true });
      await expect(audio).toHaveAttribute('preload', 'none');
      expect(await audio.evaluate((element: HTMLAudioElement) => element.paused)).toBe(true);
      const playback = await playRealAudio(page, 'audio[aria-label="Audio: voice-message.webm"]');
      expect(traffic.authorizations - baseline.authorizations).toBe(1);
      await page.setViewportSize({ width: 390, height: 844 });
      await assertContained(page, 'audio[aria-label="Audio: voice-message.webm"]');
      await assertContained(page, '[title="Send message"]');
